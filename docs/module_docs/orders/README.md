...
*   **一句话描述:** 处理买家和卖家视角的订单全生命周期管理，包括订单列表展示、订单详情查看、订单操作（如取消、提交需求、评价）、**售后处理（申请、查看、跟踪）**、**订单需求管理**、**平台介入处理**以及相关的本地缓存和状态管理。

*   **模块完整性分析 (2024-12-19 更新):**
    *   **前端实现度:** 约70-75% (核心功能已完成，部分高级功能待实现)
    *   **前后端对比:** 后端API设计完整，前端存在功能缺口
    *   **开发周期预估:** 4-5周可达到100%完整度
...
*   **(可选) 主要功能点:**
    ...
    *   ✅ **售后申请与处理**
    *   ✅ **查看售后详情与进度**
    *   ❌ **订单需求管理** (买家材料补充、卖家需求审核) - **缺失重要功能**
    *   ⚠️ **平台介入处理** (API已实现但前端处理需优化)
    *   ❌ **订单日志查看** - **缺失功能**
    *   ❌ **物流跟踪** - **缺失功能**
    *   ❌ **订单提醒功能** - **缺失功能**
    *   ✅ 订单列表本地缓存与后台更新

## 1.1 前后端功能对比分析 (Backend vs Frontend Completeness)

### 后端API完整度: 95%
- **订单核心操作:** 完整实现 (14个状态流转)
- **订单需求管理:** 完整实现 (OrderDemandApiController)
- **售后处理:** 完整实现 (OrderRefundApiController)
- **评价系统:** 完整实现 (MemberEvaluateApiController，包含邀请评价)
- **平台介入:** 完整实现 (创建platform类型需求，设置标记位)

### 前端实现缺口分析:
1. **高优先级缺失 (影响核心业务流程):**
   - 订单需求管理前端界面 (买家材料补充请求、卖家需求审核)
   - 邀请评价API调用完善
   - 平台介入处理逻辑优化
...
完整的订单状态流转图和角色权限矩阵请参考: `current_implementation_todo.md` 中的Mermaid图表。

**核心状态流转路径:**
- **正常流程:** 创建订单 → 待付款 → 待确认 → 制作中 → 待发货 → 待收货 → 已完成
- **修改流程:** 制作中 → 待买家确认修改 → 制作中
- **争议流程:** 任意状态 → 争议中 → 平台介入 → 争议解决/取消
- **售后流程:** 已完成 → 申请退款/退换货 → 售后处理完成
...
*   **接口列表与目的:**
    *   **订单核心 API:**
        ...
        *   `POST /api/shop/evaluate/invite` (✅ 后端已实现，⚠️ 前端待完善)
    ...
    *   **订单需求管理 API (❌ 前端未实现):**
        *   `POST /api/project/orderDemand/list` (查询订单需求列表)
        *   `POST /api/project/orderDemand/detail` (查询订单需求详情)
        *   `POST /api/project/orderDemand/buyerReply` (买家回复需求)
        *   `POST /api/project/orderDemand/sellerReview` (卖家审核需求)
        *   `POST /api/project/orderDemand/platformIntervention` (平台介入处理)
...
*   **API实现状态总结:**
    *   ✅ **完全实现:** 订单核心操作、售后处理
    *   ⚠️ **部分实现:** 评价系统 (邀请评价功能待完善)
    *   ❌ **未实现:** 订单需求管理、物流跟踪、订单日志

*   **细节参考:** 详细的请求/响应结构、认证要求等请参考 `api_analysis_correction.md` 和全局的 `docs/api_usage_summary_cn.md` 文档。
...
*   **高优先级问题 (影响核心功能):**
    *   **❌ 订单需求管理前端缺失:** 买家无法进行材料补充请求，卖家无法进行需求审核
        *   **影响:** 阻塞核心业务流程，影响用户体验
        *   **解决方案:** 实现 OrderDemand 相关前端界面和状态管理 (预计2周)
    *   **⚠️ 邀请评价功能不完整:** 后端API完整但前端调用逻辑待优化
        *   **影响:** 评价流程不完整
        *   **解决方案:** 完善前端邀请评价调用逻辑 (预计3天)
    *   **⚠️ 平台介入处理逻辑:** 理解偏差已修正，需要优化前端处理逻辑
        *   **影响:** 争议处理流程不够清晰
        *   **解决方案:** 优化平台介入的前端处理和界面展示 (预计1周)

*   **中优先级问题 (影响用户体验):**
    *   订单列表在快速滚动或频繁切换筛选条件时，可能因并发请求或状态更新不及时导致显示异常
    *   部分UI细节（如Timeline样式、按钮交互、售后流程页面）可能与最终设计稿存在差异
    *   图片加载可能因URL问题（如相对路径）失败
...
*   **开发者注意事项:**
    *   修改订单状态流转逻辑时，需同时考虑买家和卖家视图，**以及关联的售后状态和需求状态**，并与后端API定义保持一致
    *   平台介入通过设置标记位而非修改订单状态，注意处理逻辑的正确性
    *   测试时注意覆盖不同的订单状态、**售后类型和状态、需求处理流程**以及边缘场景

### 10. 开发优先级和里程碑 (Development Priorities & Milestones)

详细的开发计划请参考 `current_implementation_todo.md` 文档。

**里程碑总览:**
- **里程碑1 (1周):** 核心功能修复 - 邀请评价、平台介入优化
- **里程碑2 (2周):** 需求管理实现 - 买家材料补充、卖家需求审核
- **里程碑3 (1周):** 状态追踪 - 需求状态、订单统计
- **里程碑4 (1周):** 功能完善 - UI优化、集成测试

### 11. 跨模块通信机制 (Cross-Module Communication)
...
### 12. 预览与测试入口 (Preview & Testing Entry Points)
