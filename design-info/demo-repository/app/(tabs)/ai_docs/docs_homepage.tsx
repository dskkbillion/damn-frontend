...
import NewProfile1 from "./create_profile-1";
import NewProfile2 from "./create_profile-2";
...
export default function DocProfileScreen() {
  const user_id =
    userDataset[Math.floor(Math.random() * userDataset.length)].user_id;
  const user = userDataset.find((p) => p.user_id === user_id);
  const portrait =
    user?.portraits[Math.floor(Math.random() * user?.portraits.length)];
  const applications = portrait?.applications;
  ...
  function handlePortraitChange(portrait) {
    console.log("sajio");
    return <SelectProfile portrait={portrait} />;
  }
      ...
      >
        ...
        >
          <YStack>
            {isGeneratingMode ? (
              ...
              <SelectProfile portrait={user?.portraits!} />
            )}
          ...
          <XStack space="$4">
            <Label>{isGeneratingMode ? "生成模式" : "编辑模式"}</Label>
            <Switch
              size="$3"
        ...
        {isGeneratingMode ? (
          <XStack marginLeft="$8" paddingHorizontal={page_margin}>
            <Paragraph numberOfLines={1} space="$2" theme="alt2">
              <Paragraph>
                {"# " + user?.portraits[0]?.profile?.university}
              </Paragraph>
              <Paragraph>
                {"# 绩点" + user?.portraits[0]?.profile?.gpa}
              </Paragraph>
              {user?.portraits[0]?.profile?.interships.map((item, index) => (
        ...
        <XStack marginTop="$5">
          {isGeneratingMode ? (
            ...
            <NewProfile1 onValueChange={handlePageChange} />
          ) : (
            <NewProfile2 onValueChange={handlePageChange} />
          )}
        </XStack>
      </YStack>
    </Theme>
...
const RenderGeneratingMode = ({ user }) => {
  const applications = user?.portraits[0]?.applications;
    ...
    <Theme name="light">
      <YStack paddingHorizontal={page_margin}>
        ...
        >
          <Card.Header>
            {/* -----------未添加项目----------- */}
            {!isaddProgram && <Paragraph color="white"> 生成历史</Paragraph>}
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <Fieldset gap="$4" horizontal>
            ...
            <Input flex={1} id="major" placeholder="文书、推荐信..." />
            {/* <SelectDemoItem /> */}
          </Fieldset>
          <Fieldset gap="$4" horizontal>
            <Input
