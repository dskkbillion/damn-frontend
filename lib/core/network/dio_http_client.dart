...
class DioHttpClient implements IHttpClient {
  ...
  Future<Map<String, dynamic>> postMultipart(
    ...
    try {
      ...
      const String hardcodedAuthToken =
          'eyJhbGciOiJIUzUxMiJ9.eyJsb2dpbl91c2VyX2tleSI6IjMwZmZjY2YxLWFjNDUtNGM3OS04MjJiLTliNzM0MDZjZjdkYiJ9.g0FkPdnBuvpsirksABX04FrQLTjn-qgbLwRE9QLJOW6Df5syAdTGLn0IhpUYMDRaefbFQ49MWnL5wYUMRtMuiQ';
      ...
      final response =
          await _dio.post(pathOrUrl, data: formData, options: options);
      return _handleResponse(response);
    } on DioException catch (e) {
      throw _handleDioError(e);
    } catch (e) {
      throw ServerException(
          message: 'Unexpected error during Multipart POST: ${e.toString()}');
    }
  }
  ...
  Map<String, dynamic> _handleResponse(Response response) {
    ...
    if (response.statusCode! >= 200 && response.statusCode! < 300) {
      ...
      throw ServerException(
        message:
            '${response.statusCode}: ${response.data?['message'] ?? response.statusMessage ?? 'Unknown server error'}',
      );
    }
  }
