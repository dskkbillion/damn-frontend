  ...
  Future<void> onRequest(RequestOptions options, RequestInterceptorHandler handler) async {
    ...
    if (!options.headers.containsKey('Authorization')) {
      ...
      if (token != null && token.isNotEmpty) {
          ...
          AppLogger.d('[HeaderInterceptor] 成功添加Authorization头: ${token.substring(0, 15)}...');
      } else {
      ...
      AppLogger.d('[HeaderInterceptor] Authorization头已存在，值为: ${options.headers['Authorization']}');
    }

    AppLogger.d('[HeaderInterceptor] 最终headers: ${options.headers}'); 
