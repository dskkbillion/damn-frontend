...
class _AfterSalesApplyPageState extends State<AfterSalesApplyPage> {
  ...
  Future<void> _submitApplication() async {
    ...
    final imagePaths =
        _selectedImages.map((result) => result.finalFile.path).toList();
    var uploadedImageUrls = imagePaths;
    ...
    if (imagePaths.isNotEmpty) {
      ...
      final uploadResult =
          await getIt<IFileUploadService>().uploadFiles(imagePaths);
      if (!mounted) {
        return;
      }
      ...
      if (uploadError != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('凭证上传失败：$uploadError'),
    ...
    final refundAmount = widget.afterSalesType == 'REFUND'
  ...
  Future<void> _pickImages() async {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(AppLocalizations.of(context)
                .after_sales_max_images(_maxImages))),
      );
      return;
    }
    ...
    try {
      ...
      if (results.isNotEmpty) {
        ...
        if (successCount > 0) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(AppLocalizations.of(context)
                  .after_sales_image_process_success(
        ...
        if (errorCount > 0) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(AppLocalizations.of(context)
                  .after_sales_image_process_failed(errorCount)),
              backgroundColor: Theme.of(context).colorScheme.error,
            ),
          );
        }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(AppLocalizations.of(context)
                .after_sales_image_pick_failed(e.toString()))),
      );
    } finally {
      setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocConsumer<AfterSalesBloc, AfterSalesState>(
        listener: (context, state) {
          if (state is AfterSalesActionSuccess) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                  content: Text(state.errorMessage ?? '提交申请失败'),
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  validator: (value) => value == null || value.isEmpty
                      ? s.after_sales_select_reason_validator
                      : null,
                ...
                TextFormField(
                  ...
                  textInputAction:
                      TextInputAction.newline, // Suggest newline action
                  validator: (value) {
                ...
                Text(s.after_sales_upload_evidence(_maxImages),
              ...
              children: [
                ...
                Positioned(
                  ...
                  child: IconButton(
                    ...
                    tooltip:
                        AppLocalizations.of(context).after_sales_remove_image,
