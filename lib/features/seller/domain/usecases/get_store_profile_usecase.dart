import 'package:dartz/dartz.dart';
import 'package:dskk_flutter_refactor/core/error/failures.dart';
import 'package:dskk_flutter_refactor/core/usecases/usecase.dart';
import 'package:dskk_flutter_refactor/features/seller/domain/entities/seller_store_profile.dart';
import 'package:dskk_flutter_refactor/features/seller/domain/repositories/i_seller_repository.dart';
import 'package:injectable/injectable.dart';

/// 获取店铺资料UseCase
@injectable
class GetStoreProfileUseCase implements UseCase<SellerStoreProfile, NoParams> {
  final ISellerRepository _sellerRepository;

  /// 构造函数
  GetStoreProfileUseCase(this._sellerRepository);

  @override
  Future<Either<Failure, SellerStoreProfile>> call(NoParams params) {
    return _sellerRepository.getStoreProfile();
  }
} 