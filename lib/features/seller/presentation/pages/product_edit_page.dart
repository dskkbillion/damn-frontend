...
class ValidationConstants {
  static const double maxPrice = 3000; // MVP 单笔积分额度
...
class _ProductEditPageState extends State<ProductEditPage> {
  ...
  InputDecoration get _lightBorderDecoration => InputDecoration(
        ...
        focusedBorder: OutlineInputBorder(
          ...
          borderSide: const BorderSide(
              color: AppColors.sellerAccent, width: AppDimensions.borderThick),
        ),
  ...
  void initState() {
    ...
    print(
        '[ProductEditPage] initState called with productId: ${widget.productId}, isPreviewMode: ${widget.isPreviewMode}');
    ...
    // 解析商品ID
    final productIdInt =
        widget.productId != null ? int.tryParse(widget.productId!) : null;
    print('[ProductEditPage] Parsed productId as int: $productIdInt');

    // 初始化页面
    print(
        '[ProductEditPage] Initializing ProductEdit with productId: $productIdInt, isPreviewMode: ${widget.isPreviewMode}');
    ...
    _bloc.stream.listen((state) {
      print(
          '[ProductEditPage] State changed - isLoading: ${state.isLoading}, hasProduct: ${state.product != null}, isPreviewMode: ${widget.isPreviewMode}');
      if (!state.isLoading && state.product != null) {
        print(
            '[ProductEditPage] Syncing data from state - productName: ${state.formData.name}');
  ...
  /// 添加买家信息项
  void _addBuyerInfoItem(
      BuyerInfoType type, String label, String description, bool isRequired) {
    setState(() {
  ...
  /// 更新买家信息项
  void _updateBuyerInfoItem(
      int index, String label, String description, bool isRequired) {
    setState(() {
  ...
  /// 更新成功案例 - 简化版本
  void _updateSuccessCase(String caseId,
  ...
  void didUpdateWidget(ProductEditPage oldWidget) {
    ...
    if (oldWidget.productId != widget.productId) {
      print(
          '[ProductEditPage] Product ID changed from ${oldWidget.productId} to ${widget.productId}, reinitializing...');
  ...
  void _syncAdditionalFormData() {
    ...
    // 同步买家信息列表
    final buyerInfoJson = _buyerInfoItems
        .map((item) => {
              'type': item.type.name,
              'label': item.label,
        ...
        .toList();
    _bloc.add(
        UpdateFormField(fieldName: 'buyerInfoItems', value: buyerInfoJson));
    ...
    return showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text(AppLocalizations.of(context)
                .seller_product_edit_unsaved_changes_title ??
            'Unsaved Changes Detected'),
        content: Text(AppLocalizations.of(context)
                .seller_product_edit_unsaved_changes_message ??
            'You have unsaved content. Save as draft?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(null),
            child: Text(
                AppLocalizations.of(context).seller_product_edit_cancel ??
                    'Cancel'),
          ),
          TextButton(
            onPressed: () => Navigator.of(context).pop(false),
            child: Text(
                AppLocalizations.of(context).seller_product_edit_discard ??
                    'Discard'),
          ),
          ElevatedButton(
            ...
            child: Text(AppLocalizations.of(context).product_edit_save_draft ??
                'Save Draft'),
          ),
        ],
      ),
  ...
  void _syncDataFromState(ProductFormData formData) {
    print(
        '[ProductEditPage] _syncDataFromState called with name: "${formData.name}", description: "${formData.description}", variants count: ${formData.variants.length}');
    print(
        '[ProductEditPage] isPreviewMode: ${widget.isPreviewMode}, productId: ${widget.productId}');
    print(
        '[ProductEditPage] Current controller values - name: "${_nameController.text}", description: "${_descriptionController.text}"');
    ...
    if (mounted && _isInitialDataLoad) {
      ...
      print(
          '[ProductEditPage] Initialized text controllers - name: "${_nameController.text}", description: "${_descriptionController.text}"');
    }

    // 如果有变体数据，更新到本地服务档位（编辑模式和预览模式都需要）
    if (formData.variants.isNotEmpty &&
        (!_bloc.state.isCreateMode || widget.isPreviewMode)) {
      print(
          '[ProductEditPage] Updating service tiers from ${formData.variants.length} variants');
      for (int i = 0; i < formData.variants.length; i++) {
        final variant = formData.variants[i];
        print(
            '[ProductEditPage] Variant $i: name="${variant.name}", price=${variant.price}, sellingPrice=${variant.sellingPrice}');
      }

      if (mounted) {
        setState(() {
          _serviceTiers =
              ProductServiceTiers.fromProductOptionValues(formData.variants);
        });
        print('[ProductEditPage] Updated service tiers from variants:');
        print(
            '[ProductEditPage] - Basic: price=${_serviceTiers.basic.price}, delivery=${_serviceTiers.basic.deliveryDay}');
        print(
            '[ProductEditPage] - Standard: price=${_serviceTiers.standard.price}, delivery=${_serviceTiers.standard.deliveryDay}');
        print(
            '[ProductEditPage] - Premium: price=${_serviceTiers.premium.price}, delivery=${_serviceTiers.premium.deliveryDay}');
      }
    }
        ...
        setState(() {
          ...
          for (final qaMap in formData.qaList) {
            try {
              ...
              print(
                  '[ProductEditPage] Added QA item: question="${qaMap['question']}", answer="${qaMap['answer']}"');
            } catch (e) {
              print(
                  '[ProductEditPage] Error parsing QA data: $e, qaMap: $qaMap');
            }
          }
          // 清理未使用的QA控制器
          _cleanupUnusedQAControllers();
          print(
              '[ProductEditPage] Synced ${_qaList.length} QA items from formData');
          ...
          for (final buyerInfoMap in formData.buyerInfoItems) {
            try {
              ...
              print(
                  '[ProductEditPage] Added buyer info item: type="${buyerInfoMap['type']}", label="${buyerInfoMap['label']}"');
            } catch (e) {
              print(
                  '[ProductEditPage] Error parsing buyer info data: $e, buyerInfoMap: $buyerInfoMap');
            }
          }
          print(
              '[ProductEditPage] Synced ${_buyerInfoItems.length} buyer info items from formData');
      ...
      if (!replaceExisting && _bloc.state.selectedImagePaths.isNotEmpty) {
        ...
        if (combinedPaths.length > 9) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                  AppLocalizations.of(context).seller_product_edit_max_images ??
                      'Maximum 9 images allowed'),
              backgroundColor: AppColors.warning,
            ),
          );
          _bloc.add(
              SelectProductImages(imagePaths: combinedPaths.sublist(0, 9)));
        } else {
  ...
  void _submitForm() {
    ...
    if (_nameController.text.trim().isEmpty) {
      setState(() {
        _formErrors['name'] = AppLocalizations.of(context)
    ...
    if (_descriptionController.text.trim().isEmpty) {
      setState(() {
        _formErrors['description'] = AppLocalizations.of(context)
    ...
    if (_bloc.state.selectedImagePaths.isEmpty &&
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              AppLocalizations.of(context).product_edit_at_least_one_image ??
    ...
    final tiers = [
      _serviceTiers.basic,
      _serviceTiers.standard,
      _serviceTiers.premium
    ];
    for (int i = 0; i < tiers.length; i++) {
      final tier = tiers[i];
      if (tier.price <= 0) {
        setState(() {
          _formErrors['price_${tier.tier.name}'] = AppLocalizations.of(context)
                  .seller_product_edit_price_required(tier.tier.displayName) ??
              '${tier.tier.displayName} price must be greater than 0';
        });
        isValid = false;
      }
    }
    ...
    if (!isValid) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(AppLocalizations.of(context)
                  .seller_product_edit_form_incomplete ??
    ...
    // 设置基础价格为基础档的价格
    _bloc.add(
        UpdateFormField(fieldName: 'price', value: _serviceTiers.basic.price));
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(widget.isPreviewMode
              ? AppLocalizations.of(context).product_edit_preview_product ??
                  'Preview Product'
              : (widget.productId == null
                  ? AppLocalizations.of(context).product_edit_publish_product ??
                      'Publish Product'
                  : AppLocalizations.of(context).product_edit_title_edit ??
                      'Edit Product')),
          leading: IconButton(
              ...
              : [
                  ...
                  BlocBuilder<ProductEditBloc, ProductEditState>(
                    ...
                    builder: (context, state) {
                      // 判断是否是草稿商品
                      final isDraft =
                          state.product?.status == ProductStatus.draft;
                          ...
                          TextButton.icon(
                            onPressed: state.isSavingDraft
                                ...
                                : () {
                                    ...
                                    // 添加调试日志
                                    print(
                                        '[ProductEditPage] Saving draft with price: ${_serviceTiers.basic.price}');
                                    print(
                                        '[ProductEditPage] Variants count: ${variants.length}');
                                    for (var i = 0; i < variants.length; i++) {
                                      print(
                                          '[ProductEditPage] Variant $i: ${variants[i].name} - price: ${variants[i].sellingPrice}');
                                    }
                            ...
                            label: Text(state.hasUnsavedChanges
                                ? (AppLocalizations.of(context)
                                        .seller_product_edit_draft_unsaved_label ??
                                    'Draft*')
                                : (AppLocalizations.of(context)
                                        .seller_product_edit_draft_label ??
                                    'Draft')),
                            style: TextButton.styleFrom(
            ...
            listener: (context, state) {
              if (state.hasError) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(state.errorMessage ??
                        (AppLocalizations.of(context)
                                .seller_product_edit_operation_failed ??
                            'Operation failed')),
                    backgroundColor: AppColors.error,
                  ),
                );
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(AppLocalizations.of(context)
                            .seller_product_edit_draft_saved ??
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(state.isCreateMode
                        ? (AppLocalizations.of(context)
                                .seller_product_edit_publish_success ??
                            'Service published successfully!')
                        : (AppLocalizations.of(context)
                                .seller_product_edit_update_success ??
            ...
            builder: (context, state) {
              if (state.isLoading) {
                return const SellerPageSkeleton(
                    variant: SellerSkeletonVariant.form);
              }
        ...
        children: [
                ...
                TextField(
                  ...
                  decoration: _lightBorderDecoration.copyWith(
                    hintText: AppLocalizations.of(context)
                            .seller_product_edit_service_name_hint ??
                        'Service Name',
          ...
          Padding(
            ...
            child: TextField(
              ...
              decoration: _lightBorderDecoration.copyWith(
                hintText: AppLocalizations.of(context)
                        .seller_product_edit_description_hint ??
                    'Describe your service details...',
                hintStyle: const TextStyle(color: AppColors.textTertiary),
        ...
        children: [
          ...
          Padding(
            ...
            child: Align(
              ...
              child: Text(
                AppLocalizations.of(context).product_edit_service_tiers ??
                    ...
                    children: [
                      ...
                      if (_qaList.isNotEmpty)
                        Container(
                          ...
                          child: Text(
                            AppLocalizations.of(context)
                                    .seller_product_edit_faq_count(
                                        _qaList.length) ??
                                '${_qaList.length} questions',
                            style: const TextStyle(
                              fontSize: 12,
            ...
            if (!widget.isPreviewMode)
              Container(
                ...
                child: OutlinedButton.icon(
                  ...
                  label: Text(AppLocalizations.of(context)
                          .seller_product_edit_add_question ??
                      'Add Question'),
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
                ...
                child: TextField(
                  ...
                  decoration: _lightBorderDecoration.copyWith(
                    labelText: AppLocalizations.of(context)
                            .seller_product_edit_question_label ??
                        'Question',
                    hintText: AppLocalizations.of(context)
                            .seller_product_edit_question_hint ??
              ...
              if (!widget.isPreviewMode)
                IconButton(
                  ...
                  tooltip: AppLocalizations.of(context)
                          .seller_product_edit_delete_question ??
                      'Delete Question',
                ),
            ],
          ),
          ...
          TextField(
            ...
            decoration: _lightBorderDecoration.copyWith(
              labelText: AppLocalizations.of(context)
                      .seller_product_edit_answer_label ??
                  'Answer',
              hintText: AppLocalizations.of(context)
                      .seller_product_edit_answer_hint ??
                  'Enter the answer',
            ),
        ...
        children: [
            ...
            child: Container(
              ...
              child: Row(
                ...
                children: [
                  Text(
                    AppLocalizations.of(context)
                            .seller_product_edit_buyer_info_title ??
                    ...
                    children: [
                      ...
                      if (_buyerInfoItems.isNotEmpty)
                        Container(
                          ...
                          child: Text(
                            AppLocalizations.of(context)
                                    .seller_product_edit_buyer_info_count(
                                        _buyerInfoItems.length) ??
                                '${_buyerInfoItems.length} items',
                            style: const TextStyle(
                              fontSize: 12,
                              color: AppColors.sellerAccent,
                            ),
          ...
          if (!_isBuyerInfoExpanded)
            Container(
              ...
              child: Text(
                AppLocalizations.of(context)
                        .seller_product_edit_buyer_info_desc ??
                    'Select the information types buyers need to provide',
                style: const TextStyle(
                  fontSize: 14,
          ...
          if (_isBuyerInfoExpanded) ...[
            ...
            if (_buyerInfoItems.isNotEmpty) ...[
              ...
              Text(
                AppLocalizations.of(context)
                        .seller_product_edit_selected_items ??
                    'Selected items:',
                style:
              ...
              ..._buyerInfoItems
                  .asMap()
                  .entries
                  .map((entry) => _buildBuyerInfoItem(entry.key, entry.value)),
            ],
          ],
        ],
      ),
  ...
  Widget _buildBuyerInfoItem(int index, BuyerInfoItem item) {
    return Container(
      ...
      child: Row(
        children: [
          Icon(_getIconForInfoType(item.type),
              size: 20, color: AppColors.sellerAccent),
                  ...
                  children: [
                    ...
                    if (item.isRequired)
                      Container(
                        ...
                        child: Text(
                          AppLocalizations.of(context)
                                  .seller_product_edit_required ??
                              'Required',
                          style:
                              TextStyle(fontSize: 10, color: AppColors.error),
                        ),
                      ),
                  ],
                ),
          ...
          Row(
            ...
            children: [
              IconButton(
                ...
                tooltip: AppLocalizations.of(context)
                        .seller_product_edit_edit_tooltip ??
                    'Edit',
              ),
              IconButton(
                ...
                tooltip: AppLocalizations.of(context)
                        .seller_product_edit_delete_tooltip ??
                    'Delete',
              ),
            ],
          ),
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          title: Text(AppLocalizations.of(context)
                  .seller_product_edit_add_info_title(type.displayName) ??
              'Add ${type.displayName} Info'),
              ...
              child: Column(
                ...
                children: [
                  TextField(
                    ...
                    decoration: _lightBorderDecoration.copyWith(
                      labelText: AppLocalizations.of(context)
                              .seller_product_edit_info_label ??
                          'Info Label',
                      hintText: AppLocalizations.of(context)
                              .seller_product_edit_info_label_hint ??
                  ...
                  TextField(
                    ...
                    decoration: _lightBorderDecoration.copyWith(
                      labelText: AppLocalizations.of(context)
                              .seller_product_edit_info_description ??
                          'Detailed Description',
                      hintText: AppLocalizations.of(context)
                              .seller_product_edit_info_description_hint ??
                  ...
                  CheckboxListTile(
                    title: Text(AppLocalizations.of(context)
                            .seller_product_edit_required_field ??
                        'Required Field'),
                    value: isRequired,
                    onChanged: (value) {
                      setState(() {
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              child: Text(
                  AppLocalizations.of(context).seller_product_edit_add ??
                      'Add'),
            ),
          ],
        ),
  ...
  void _showEditBuyerInfoDialog(int index, BuyerInfoItem item) {
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          title: Text(AppLocalizations.of(context)
                  .seller_product_edit_edit_info_title(item.type.displayName) ??
              'Edit ${item.type.displayName} Info'),
          content: ConstrainedBox(
            constraints: BoxConstraints(
              maxHeight: MediaQuery.of(context).size.height * 0.5, // 限制最大高度
              maxWidth: double.maxFinite,
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              child: Text(
                  AppLocalizations.of(context).seller_product_edit_save ??
                      'Save'),
            ),
          ],
        ),
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text(
                AppLocalizations.of(context).product_edit_success_cases ??
                    'Success Cases',
                style: const TextStyle(
                  fontSize: 16,
                  fontWeight: FontWeight.bold,
              ...
              if (state.successCases.isNotEmpty)
                Container(
                  ...
                  child: Text(
                    AppLocalizations.of(context)
                            .seller_product_edit_cases_count(
                                state.successCases.length) ??
                        '${state.successCases.length} cases',
                    style: const TextStyle(
                      fontSize: 12,
                      color: AppColors.sellerAccent,
                    ),
  ...
  Widget _buildAddSuccessCaseButton() {
    return InkWell(
      ...
      child: Container(
        decoration: BoxDecoration(
          border: Border.all(
              color: AppColors.borderInput, style: BorderStyle.solid),
          borderRadius: BorderRadius.circular(8),
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              AppLocalizations.of(context).seller_product_edit_add_case ??
                  'Add Case',
  ...
  /// 构建编辑对话框的图片
  Widget _buildEditDialogImage(
      String? localPath, String? imageUrl, bool imageChanged) {
    // 如果图片已更改，只显示本地图片
        ...
        errorBuilder: (context, error, stackTrace) {
          return Column(
            ...
            children: [
              ...
              Text(
                  AppLocalizations.of(context)
                          .seller_product_edit_image_load_failed ??
                      'Image load failed',
                  style:
    ...
    return Column(
      ...
      children: [
        ...
        Text(
            AppLocalizations.of(context)
                    .seller_product_edit_click_select_image ??
          ...
          Center(
            child: Column(
              ...
              children: [
                ...
                Text(
                    AppLocalizations.of(context)
                            .seller_product_edit_upload_failed ??
                        'Upload Failed',
                    style: TextStyle(color: AppColors.error, fontSize: 12)),
                if (successCase.canRetry)
                  TextButton(
                    onPressed: () => _bloc
                        .add(RetrySuccessCaseUpload(caseId: successCase.id)),
                    child: Text(
                        AppLocalizations.of(context)
                                .seller_product_edit_upload_retry ??
                            'Retry',
                ...
                Positioned(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      InkWell(
                        onTap: () =>
                            _showEditSuccessCaseDialog(state, successCase.id),
                        child: Container(
  ...
  void _showAddSuccessCaseDialog() {
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          title: Text(
              AppLocalizations.of(context).product_edit_add_success_case ??
                  'Add Success Case'),
          content: ConstrainedBox(
            constraints: BoxConstraints(
              maxHeight:
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: _lightBorderDecoration.copyWith(
                      labelText: AppLocalizations.of(context)
                              .seller_product_edit_case_title_label ??
                          'Case Title',
                      hintText: AppLocalizations.of(context)
                              .seller_product_edit_case_title_hint ??
                  ...
                  TextField(
                    ...
                    decoration: _lightBorderDecoration.copyWith(
                      labelText: AppLocalizations.of(context)
                              .seller_product_edit_case_desc_label ??
                          'Case Description',
                      hintText: AppLocalizations.of(context)
                              .seller_product_edit_case_desc_hint ??
              ...
              onPressed: () {
                if (selectedImagePath != null &&
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                        content: Text(AppLocalizations.of(context)
                                .seller_product_edit_select_image_and_title ??
  ...
  void _showEditSuccessCaseDialog(ProductEditState state, String caseId) {
    ...
    final descriptionController =
        TextEditingController(text: successCase.description);
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          title: Text(
              AppLocalizations.of(context).product_edit_edit_success_case ??
                  'Edit Success Case'),
          content: ConstrainedBox(
            constraints: BoxConstraints(
              maxHeight:
                ...
                children: [
                  ...
                  InkWell(
                    ...
                    child: Container(
                      ...
                      child: ClipRRect(
                        borderRadius: BorderRadius.circular(8),
                        child: _buildEditDialogImage(
                            selectedImagePath, currentImageUrl, imageChanged),
                      ),
                    ),
                  ),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Text(
                AppLocalizations.of(context).seller_product_edit_cover_image ??
                    'Service Cover Image',
                style: TextStyle(
                  fontSize: 16,
                  fontWeight: FontWeight.bold,
          ...
          if (state.uploadStatus == UploadStatus.failure &&
            ...
            Padding(
              ...
              child: Text(
                AppLocalizations.of(context).seller_product_edit_upload_error(
                        state.errorMessage ?? '') ??
          ...
          Padding(
            ...
            child: Text(
              AppLocalizations.of(context)
                      .seller_product_edit_image_format_hint ??
      ...
      return Container(
        ...
        child: Row(
          ...
          children: [
            ...
            const SizedBox(width: 8),
            Text(
              AppLocalizations.of(context).product_edit_uploading_progress(
                  state.uploadedCount, state.totalUploadCount),
              style: TextStyle(
                fontSize: 12,
      ...
      return Container(
        ...
        child: Row(
          ...
          children: [
            ...
            const SizedBox(width: 8),
            Text(
              AppLocalizations.of(context).seller_product_edit_upload_success ??
                  'Upload Successful',
              style: TextStyle(
                fontSize: 12,
      ...
      return Container(
        ...
        child: Row(
          ...
          children: [
            ...
            const SizedBox(width: 8),
            Text(
              AppLocalizations.of(context).seller_product_edit_upload_failed ??
                  'Upload Failed',
              style: const TextStyle(
                fontSize: 12,
                color: AppColors.error,
  ...
  // 构建单个图片项
  Widget _buildImageItem(ProductEditState state, int index, String? localPath,
      String? networkUrl) {
    return GestureDetector(
          ...
          if (index == 0)
            Positioned(
              ...
              child: Container(
                ...
                child: Text(
                  AppLocalizations.of(context).seller_product_edit_main_image ??
                      'Main',
                  style:
                      const TextStyle(color: AppColors.onPrimary, fontSize: 10),
                ),
              ),
            ),
        ],
    ...
    return InkWell(
      ...
      child: Container(
        decoration: BoxDecoration(
          border: Border.all(
            color:
                canAdd ? AppColors.borderInput : AppColors.backgroundSecondary,
            width: 1,
          ),
          borderRadius: BorderRadius.circular(8),
          color:
              canAdd ? AppColors.backgroundCard : AppColors.backgroundSecondary,
        ),
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            ...
            Text(
              isUploading
                  ? (AppLocalizations.of(context).product_edit_uploading ??
                      'Uploading...')
                  : (AppLocalizations.of(context).product_edit_add_image ??
                      'Add Image'),
              style: TextStyle(
                fontSize: 12,
          ...
          child: Column(
            ...
            children: [
              ...
              if (hasPrice) ...[
                ...
                Text(
                  PriceFormatter.format(tierConfig.price),
                  style: TextStyle(
                    fontSize: 12,
  ...
  Widget _buildTierPriceEditor() {
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        labelText: AppLocalizations.of(context)
                .seller_product_edit_tier_price_label(
                    _selectedTier.displayName) ??
            '${_selectedTier.displayName} Price',
        ...
        helperText: AppLocalizations.of(context).seller_product_edit_max_price(
                '${ValidationConstants.maxPrice}') ??
            'Maximum: ${ValidationConstants.maxPrice}',
      ...
      onChanged: (value) {
        setState(() {
          ...
          if (price > ValidationConstants.maxPrice) {
            _formErrors['price_${_selectedTier.name}'] =
                AppLocalizations.of(context)
                        .seller_product_edit_price_exceed_max(
                            '${ValidationConstants.maxPrice}') ??
                    'Price cannot exceed ${ValidationConstants.maxPrice}';
          } else if (price > 0 && price < 1) {
            _formErrors['price_${_selectedTier.name}'] = '最低价格为 1 积分';
          } else {
  ...
  /// 获取或创建QA控制器
  TextEditingController _getQAController(
      int qaIndex, String type, String initialValue) {
