...
class _SellerProfilePageState extends ConsumerState<SellerProfilePage> {
  ...
  void initState() {
    ...
    _onlineStatusSubscription =
        EventBus().sellerOnlineStatusChangedStream.listen((event) {
      if (mounted) {
        setState(() {
          _isOnline = event.isOnline;
        });
        AppLogger.d(
            '[SellerProfilePage] Online status updated via EventBus: ${event.isOnline}');
      }
    });
  }
  ...
  Future<void> _loadOnlineStatus() async {
    try {
      ...
      result.fold(
        (failure) {
          AppLogger.d(
              '[SellerProfilePage] Failed to load online status: ${failure.message}');
        },
        (settings) {
          if (mounted) {
            setState(() {
            ...
            AppLogger.d(
                '[SellerProfilePage] Initial online status loaded: ${settings.isOnline}');
          }
        },
      );
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    AppLogger.d(
        '[SellerProfilePage] Current ProfileBloc state: ${currentState.runtimeType}');

    // 只在真正需要时才触发初始化
    if (currentState is ProfileInitial) {
      AppLogger.d(
          '[SellerProfilePage] ProfileBloc is in Initial state, triggering CheckAuthStatus');
      profileBloc.add(CheckAuthStatusEvent());
    } else if (currentState is ProfileLoaded ||
        currentState is ProfileUpdated) {
      AppLogger.d(
          '[SellerProfilePage] Profile already loaded: ${(currentState as dynamic).profile?.nickName}');
      // 已经有数据了，不需要重新加载
    }
      ...
      child: BlocListener<ProfileBloc, ProfileState>(
        listener: (context, state) {
          if (state is ProfileAuthStatusLoaded && state.isAuthenticated) {
            ...
            context
                .read<ProfileBloc>()
                .add(const GetUserProfileCachedEvent(mode: AppMode.seller));
          }
        },
            ...
            return Scaffold(
              ...
              body: GlassBackdrop(
                child: SingleChildScrollView(
                  ...
                  // 顶部安全距离属于滚动内容，而不是固定 SafeArea；卖家页
                  // 因而可和买家个人页一样，滚动经过透明状态栏区域。
                  ...
                  child: Column(
                    children: [
                      ...
                      _buildMenuSection(
                        AppLocalizations.of(context).seller_profile_my_wallet,
                      ...
                      _buildSectionTitle(
                          AppLocalizations.of(context).seller_profile_settings),
                      _buildMenuSection(
                        AppLocalizations.of(context)
                      ...
                      const SizedBox(height: 10),
                      _buildSectionTitle(
                          AppLocalizations.of(context).seller_profile_about_us),
                      _buildMenuSection(
                          AppLocalizations.of(context).seller_profile_mission,
          ...
          Row(
            children: [
              ...
              GestureDetector(
                onTap: () {
                  AppLogger.d(
                      '[SellerProfilePage] Avatar tapped, profile: $profile');
                  ...
                  if (profile?.userId != null) {
                    ...
                    AppLogger.d(
                        '[SellerProfilePage] Cannot navigate: userId is null');
                  }
                },
                child: Container(
                  width: 80,
                        ...
                        children: [
                          ...
                          Text(
                            _isOnline
                                ...
                                : AppLocalizations.of(context)
                                    .seller_profile_seller_mode_offline,
                            style: const TextStyle(
                              fontSize: 12,
                              color: AppColors.onPrimary,
                            ),
          ...
          GlassCard(
            ...
            child: Row(
              ...
              children: [
                Text(
                  _sellerModeOn
                      ? AppLocalizations.of(context).seller_profile_seller_mode
                      : AppLocalizations.of(context).seller_profile_buyer_mode,
                  style: const TextStyle(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              _buildOrderStatusItem(Icons.attach_money,
                  AppLocalizations.of(context).seller_profile_order_pending, 0),
              _buildOrderStatusItem(
                  Icons.sync,
                  AppLocalizations.of(context).seller_profile_order_processing,
                  0),
              _buildOrderStatusItem(
                  Icons.check_circle_outline,
                  AppLocalizations.of(context).seller_profile_order_delivered,
                  0),
              _buildOrderStatusItem(Icons.assignment_return_outlined,
                  AppLocalizations.of(context).seller_profile_order_refund, 0),
            ],
          ),
        ],
      ),
  ...
  Widget _buildMenuSection(String title, IconData icon, String badge,
    ...
    return GlassCard(
      margin: const EdgeInsets.symmetric(
          horizontal: AppDimensions.spacingLg, vertical: 4),
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text(AppLocalizations.of(context)
              .seller_profile_feature_not_implemented(feature))),
    );
  }
}
