...
class _ChatInputFieldState extends State<ChatInputField> {
  ...
  final AudioRecorder _audioRecorder =
      AudioRecorder(); // Instance of the recorder
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<AiChatBloc, AiChatState>(
      buildWhen: (previous, current) =>
          ...
          previous.imageUploadStates !=
              current.imageUploadStates, // Also rebuild on upload state changes
      builder: (context, state) {
        final bool isStreaming = state.status == AiChatStatus.streamingResponse;
        final bool isCancelling =
            state.status == AiChatStatus.cancellingGeneration;
        ...
        final Map<String, ImageUploadState> uploadStates =
        ...
        return ValueListenableBuilder<TextEditingValue>(
            ...
            builder: (context, textValue, child) {
              ...
              final bool baseCanSendMessage =
                  !isBusy && !(_isRecording ?? false) && (hasText || hasImages);
              ...
              return Container(
                ...
                decoration: BoxDecoration(
                  color: AppColors.backgroundCard.withValues(alpha: 0.68),
                ...
                child: Column(
                  // Use Column to stack preview above input row
                  ...
                  children: [
                    ...
                    if (pendingImages.isNotEmpty)
                      ...
                      _buildImagePreviewRow(
                          context, pendingImages, uploadStates),
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          IconButton(
                            ...
                            onPressed: isBusy || _isRecording
                                ? null
                                : _pickAndDispatchImage,
                            tooltip:
                                appLocalizations.ai_docs_add_image, // 使用国际化文本
                            ...
                            child: TextField(
                              ...
                              decoration: InputDecoration(
                                hintText: _isRecording
                                    ? appLocalizations.ai_docs_recording
                                    : appLocalizations
                                        .ai_docs_enter_message, // 使用国际化文本
                              ...
                              onSubmitted: baseCanSendMessage
                                  ? (_) => widget
                                      .onSendMessage(widget.textController.text)
                                  : null,
                          ...
                          if (isStreaming || isCancelling)
                            IconButton(
                              ...
                              tooltip: isCancelling
                                  ...
                                  : appLocalizations
                                      .ai_docs_stop_generation, // 使用国际化文本
                              onPressed: isCancelling
                                  ...
                                  : () {
                                      ...
                                      context
                                          .read<AiChatBloc>()
                                          .add(const CancelChatGeneration());
                                    },
                            )
                          else
                            IconButton(
                                        ...
                                        ScaffoldMessenger.of(context)
                                          ...
                                          SnackBar(
                                              content: Text(appLocalizations
                                                  .ai_docs_uploading_images)), // 使用国际化文本
                                        );
                                      } else {
                              ...
                              onLongPress:
                                  isBusy ? null : _showMarkdownExampleMenu,
                              tooltip: appLocalizations
                                  .ai_docs_send_message, // 使用国际化文本
                              style: IconButton.styleFrom(
  ...
  Widget _buildImagePreviewRow(BuildContext context, List<File> images,
                      ...
                      child: Center(
                        child: switch (uploadState.status) {
                          ...
                          ImageUploadStatus.success =>
                            null, // No icon needed for success
                        },
                      ),
                    ),
                  ),
                    ...
                    child: InkWell(
                      ...
                      onTap: () {
                        ...
                        context.read<AiChatBloc>().add(
                            RemovePendingImage(imagePathToRemove: filePath));
                      },
                      child: const Padding(
    ...
    try {
      ...
      if (results.isNotEmpty && mounted) {
        ...
        if (result.isSuccess) {
          ...
          context
              .read<AiChatBloc>()
              .add(PickImage(imageFile: result.finalFile));
          ...
          if (result.compressionRatio != null) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                    '图片已优化处理，压缩 ${result.compressionRatio!.toStringAsFixed(1)}%'),
  ...
  void _handleVoiceButtonPress() async {
    ...
    if (!_isRecording) {
        ...
        if (!status.isGranted) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                  content: Text(appLocalizations
                      .ai_docs_mic_permission_denied)), // 使用国际化文本
            );
          }
      ...
      final String filePath =
          '${tempDir.path}/audio_${DateTime.now().millisecondsSinceEpoch}.wav'; // 使用WAV格式
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text(appLocalizations
                    .ai_docs_recording_error(e.toString()))), // 使用国际化文本
          );
        }
      }
    } else {
      ...
      try {
        ...
        if (kDebugMode &&
            path != null &&
            (dotenv.env['ENABLE_OMNI_VOICE_MOCK'] ?? 'false').toLowerCase() ==
                'true') {
          try {
            final mockBytes =
                await rootBundle.load('assets/audio/mock_voice.wav');
            await File(path)
                .writeAsBytes(mockBytes.buffer.asUint8List(), flush: true);
            AppLogger.w(
                "[VOICE-MOCK] 已用 assets/audio/mock_voice.wav 替换录音 ($path)");
          } catch (e) {
        ...
        if (path != null) {
          ...
          if (await recordedFile.exists()) {
            AppLogger.d(
                "Recorded file size: ${await recordedFile.length()} bytes");
            ...
            if (mounted) {
              ...
              if (currentState.selectedConversationId == null) {
                // 先创建新对话，再发送语音消息
                AppLogger.d(
                    "[ChatInputField] ${appLocalizations.ai_docs_auto_create_voice}");
                aiChatBloc.add(CreateNewConversationAndSendVoiceMessage(
            ...
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                    content: Text(appLocalizations
                        .ai_docs_recording_file_not_found)), // 使用国际化文本
              );
            }
          }
        } else {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                  content: Text(appLocalizations
                      .ai_docs_stop_recording_error)), // 使用国际化文本
            );
          }
        }
      } catch (e) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text(
                    appLocalizations.ai_docs_stop_recording_error_with_reason(
                        e.toString()))), // 使用国际化文本
          );
        }
