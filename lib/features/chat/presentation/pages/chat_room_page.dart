...
class ChatRoomPage extends StatefulWidget {
  ...
  final Function(int chatId, ChatMessage? newLastMessage)?
      onMessageRevoked; // 新增消息撤回回调参数
...
class _ChatRoomPageState extends State<ChatRoomPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        AppLogger.d('[ChatRoomPage] 触发LoadChatMessages事件，chatId已省略');
  ...
  void _loadMoreMessages() {
    ...
    AppLogger.d(
        '[ChatRoomPage] Loading more messages, pageNum: ${_pageNum + 1}');
  ...
  void _scrollToBottom() {
    if (_scrollController.hasClients) {
      try {
        ...
        AppLogger.d("[ChatRoom] Error scrolling to bottom: ${e.runtimeType}");
      }
    } else {
  ...
  bool _shouldShowTimestampSeparator(
      ChatMessage current, ChatMessage? previous) {
    if (previous == null) {
      return true; // Always show for the very first message displayed
    }

    // 检查是否跨天
    final currentDate = DateTime(current.createTime.year,
        current.createTime.month, current.createTime.day);
    final previousDate = DateTime(previous.createTime.year,
        previous.createTime.month, previous.createTime.day);
  ...
  Widget _buildTimestampSeparator(DateTime timestamp, bool isFirstMessage) {
    ...
    if (messageDate.isAtSameMomentAs(today)) {
      ...
      // 昨天的消息
      formattedTime = AppLocalizations.of(context)
          .chat_yesterday_time(DateFormat('HH:mm').format(timestamp));
    } else if (timestamp.year == now.year) {
      // 今年的消息显示月日和时间
      formattedTime =
          DateFormat.MMMd(Intl.getCurrentLocale()).add_Hm().format(timestamp);
    } else {
      // 跨年的消息显示完整日期
      formattedTime =
          DateFormat.yMMMd(Intl.getCurrentLocale()).add_Hm().format(timestamp);
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        foregroundColor:
            AppColors.textPrimary, // Set AppBar foreground (text/icons)
        ...
        leading: BackButton(
          onPressed: () {
            ...
            bool chatWasViewed =
                context.read<ChatMessagesBloc>().state is ChatMessagesLoaded;
            Navigator.pop(context,
                chatWasViewed); // Return true if viewed, false otherwise
          },
        ),
          ...
          builder: (context, state) {
            if (state is ChatMessagesLoaded) {
              return Text(state.opponent.nickName ?? s.chat_unknown_user);
                ...
                state is! ChatMessagesInitial) {
              final bloc = context.read<ChatMessagesBloc>();
              if (bloc.state is ChatMessagesLoaded) {
                return Text(
                    (bloc.state as ChatMessagesLoaded).opponent.nickName ??
                        s.chat_unknown_user);
              }
                            ...
                            onProductTap: chatRoom.hasAvailableProduct
                                ? () {
                                    ...
                                    if (chatRoom.productId != null) {
                                      AppLogger.d('导航到商品详情页；商品标识和名称已省略');
                  ...
                  listener: (context, state) {
                    ...
                    if (state is ChatMessagesLoaded) {
                      ...
                      if (state.isInitialLoad) {
                        AppLogger.d(
                            '[ChatRoomPage] Messages loaded successfully, triggering onMessagesLoaded callback');
                      ...
                      if (state.hasMessageRevoked) {
                        AppLogger.d(
                            '[ChatRoomPage] Message revoked detected, triggering onMessageRevoked callback');
                        // 获取新的最后一条消息
                        final newLastMessage = state.messages.isNotEmpty
                            ? state.messages.last
                            : null;
                        widget.onMessageRevoked
                            ?.call(widget.chatId, newLastMessage);
                        ...
                        Future.delayed(const Duration(milliseconds: 100), () {
                          if (mounted) {
                            context
                                ...
                                .add(const ResetMessageRevokedFlag());
                          }
                        });
                      }
                      ...
                      if (state.hasMessageSent) {
                        AppLogger.d(
                            '[ChatRoomPage] Message sent detected, triggering onMessageSent callback');
                        ...
                        Future.delayed(const Duration(milliseconds: 100), () {
                          if (mounted) {
                            context
                                .read<ChatMessagesBloc>()
                                .add(const ResetMessageSentFlag());
                          }
                        });
                      }
                          ...
                          ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final bool showTimestamp =
                                  _shouldShowTimestampSeparator(
                                      currentMessage, previousMessage);

                              return Column(
                                children: [
                                  if (showTimestamp)
                                    _buildTimestampSeparator(
                                  ...
                                  ChatMessageBubble(
                                    ...
                                    currentUserParticipantId:
                                        state.currentUserParticipantId,
