...
class ChatPreloadService {
  static const int _maxPreloadImages =
      20; // Maximum number of images to preload
  static const int _preloadBatchSize = 5; // Number of images to preload at once
  static const Duration _preloadDelay =
      Duration(milliseconds: 500); // Delay between batches
  ...
  static const double _maxMemoryPressure =
      0.8; // Stop preloading at 80% memory usage
  ...
  void _checkMemoryPressure() {
    ...
    if (maxSize > 0) {
      ...
      if (_lastMemoryPressure > _maxMemoryPressure) {
        AppLogger.d(
            '[ChatPreload] High memory pressure detected: ${(_lastMemoryPressure * 100).toStringAsFixed(1)}%');
        _pausePreloading();
        ...
        if (_lastMemoryPressure > 0.9) {
          imageCache.clear();
          AppLogger.d(
              '[ChatPreload] Evicted image cache due to high memory pressure');
        }
      }
    }
  }
  ...
  Future<void> preloadChatRoomAvatars(
    ...
    AppLogger.d(
        '[ChatPreload] Starting to preload ${chatRooms.length} chat room avatars');

    final avatarUrls = <String>[];

    for (final room in chatRooms) {
      // Add participant avatars
      if (room.participant1.avatar != null &&
          room.participant1.avatar!.isNotEmpty) {
        avatarUrls.add(room.participant1.avatar!);
      }
      if (room.participant2.avatar != null &&
          room.participant2.avatar!.isNotEmpty) {
  ...
  Future<void> preloadMessageImages(
    ...
    if (imageUrls.isNotEmpty) {
      AppLogger.d(
          '[ChatPreload] Queuing ${imageUrls.length} message images for preload');
  ...
  void _startPreloadingFromQueue(BuildContext context,
      {bool isAvatar = false}) {
    ...
    try {
      ...
      await precacheImage(
        ...
        onError: (exception, stackTrace) {
          AppLogger.d('[ChatPreload] Failed to preload image; URL omitted');
        },
      );
      ...
      AppLogger.d(
          '[ChatPreload] Error preloading image: ${e.runtimeType}; URL omitted');
    }
  }
