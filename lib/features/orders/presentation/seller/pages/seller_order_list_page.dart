...
class _SellerOrderListPageState extends State<SellerOrderListPage>
  ...
  void initState() {
    ...
    if (widget.initialStatus != null) {
      ...
      AppLogger.d(
          '[SellerOrderListPage] Using initialStatus from URL: ${widget.initialStatus}, parsed: $initialStatus');
    }
    // 其次，尝试从Bloc状态获取
    else if (context.read<SellerOrderListBloc>().state
        is SellerOrderListSuccess) {
      initialStatus =
          (context.read<SellerOrderListBloc>().state as SellerOrderListSuccess)
              .currentStatusFilter;
      AppLogger.d(
          '[SellerOrderListPage] Using initialStatus from Bloc state: $initialStatus');
    }

    // 计算初始Tab索引
    final initialIndex = _findIndexForStatus(initialStatus);
    AppLogger.d(
        '[SellerOrderListPage] Setting initial tab index: $initialIndex for status: $initialStatus');
  ...
  void _handleTabSelection() {
    if (!_tabController.indexIsChanging) {
      ...
      context
          .read<SellerOrderListBloc>()
          .add(SellerOrderStatusFilterChanged(newStatusFilter: selectedStatus));
    }
  }
      ...
      body: BlocConsumer<SellerOrderListBloc, SellerOrderListState>(
        ...
        builder: (context, state) {
          ...
          if (state is SellerOrderListSuccess) {
            ...
            ordersToShow = state.previousState.orders;
            successState = state.previousState;
          } else if (state is SellerOrderListLoading &&
          ...
          final bool isActionInProgress =
              state is SellerOrderListActionInProgress;
          ...
          return Stack(
            // Use Stack to overlay progress indicator
            children: [
              RefreshIndicator(
                onRefresh: () async {
                  context.read<SellerOrderListBloc>().add(
                      LoadSellerOrdersRequested(
                          statusFilter: _tabStatuses[_tabController.index],
                          refresh: true));
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return SellerOrderItemCard(
                            ...
                            onTap: () {
                              final route = _resolveSellerOrderRoute(order);
                              context.push(route);
                              AppLogger.d(
                                  '[SellerOrderListPage] Pushing to $route for order ${order.id}');
                            },
                          );
                        },
                      ),
  ...
  String _resolveSellerOrderRoute(Order order) {
    final refundId = order.refundId;
    final isAfterSalesOrder = order.state == OrderStatus.afterSale ||
        order.state == OrderStatus.AfterSaleRejection ||
        order.state == OrderStatus.applyingForMediation;

    if (isAfterSalesOrder && refundId != null) {
      return '/seller/after-sales/$refundId';
    }
  ...
  List<Widget> _buildTabsWithCounts() {
    ...
    return List.generate(_tabs.length, (index) {
      ...
      if (count > 0 && status != OrderStatus.unknown) {
        // 不显示"全部"的数量
        return Tab(
          child: Row(
            mainAxisSize: MainAxisSize.min,
  ...
  String _getPendingDescription(SellerOrderListSuccess? state) {
    ...
    final awaitingStart =
        state.orders.where((o) => o.state == OrderStatus.awaitingStart).length;
    final awaitingDelivery = state.orders
        .where((o) => o.state == OrderStatus.awaitingDelivery)
        .length;
