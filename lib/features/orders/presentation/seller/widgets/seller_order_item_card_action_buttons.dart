...
class SellerOrderItemCardActionButtons extends StatelessWidget {
    ...
    result.fold(
      ...
      (chatRooms) {
        final room = chatRooms.cast<ChatRoom?>().firstWhere(
              (room) =>
                  room != null &&
                  ((room.participant1.referId == buyerId &&
                          room.participant2.referId == sellerId) ||
                      (room.participant1.referId == sellerId &&
                          room.participant2.referId == buyerId)) &&
        ...
        if (room == null) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                  '未找到该订单对应聊天室，请在卖家聊天列表中查找“${order.items.first.productName}”'),
            ),
          );
          return;
        }
  ...
  String _resolveSellerOrderRoute() {
    ...
    if (isAfterSalesOrder && order.refundId != null) {
    ...
    if (isLightConsultation) {
      ...
      switch (order.state) {
        ...
        case OrderStatus.canceled:
          ...
          buttons.add(OutlinedButton(
              onPressed: () {
                context.push(_resolveSellerOrderRoute());
              },
              style: outlineStyle,
              child: const Text('查看')));
          break;
      ...
      switch (order.state) {
          ...
          buttons.add(OutlinedButton(
              onPressed: () async {
                  ...
                  builder: (BuildContext dialogContext) {
                    return AlertDialog(
                      ...
                      actions: <Widget>[
                        ...
                        TextButton(
                          child:
                              const Text('确认拒绝'), // More specific confirmation
                          onPressed: () =>
                ...
                if (confirmed == true) {
                  ...
                  BlocProvider.of<SellerOrderListBloc>(context)
                      .add(RejectOrderRequested(orderId: order.id));
                }
              },
              style: outlineStyle,
          ...
          buttons.add(ElevatedButton(
              onPressed: () async {
                ...
                if (confirmed == true) {
                  ...
                  BlocProvider.of<SellerOrderListBloc>(context)
                      .add(ConfirmAcceptanceRequested(orderId: order.id));
                }
              },
              style: filledStyle,
        ...
        case OrderStatus.awaitingConfirmation:
          ...
          buttons.add(OutlinedButton(
              onPressed: () {
                AppLogger.d(
                    '[SellerButtons] View delivery ${order.id}'); /* TODO: Navigate/Show delivery details */
              },
              style: outlineStyle,
              child: const Text('查看交付')));
          buttons.add(ElevatedButton(
              onPressed: () {
                AppLogger.d(
                    '[SellerButtons] Remind confirmation ${order.id}'); /* TODO: Show reminder confirmation? */
              },
              style: filledStyle,
              child: const Text('提醒确认')));
          break;
        case OrderStatus.orderCompleted:
          ...
          buttons.add(OutlinedButton(
              onPressed: () async {
                ...
                if (confirmed == true) {
                  ...
                  BlocProvider.of<SellerOrderListBloc>(context)
                      .add(DeleteSellerRecordRequested(orderId: order.id));
                }
              },
              style: outlineStyle,
        ...
        case OrderStatus.canceled:
          ...
          buttons.add(OutlinedButton(
              onPressed: () async {
                ...
                if (confirmed == true) {
                  AppLogger.d(
                      '[SellerButtons] Delete cancelled record ${order.id}');
                  BlocProvider.of<SellerOrderListBloc>(context)
                      .add(DeleteSellerRecordRequested(orderId: order.id));
                }
              },
        ...
        default:
          ...
          buttons.add(OutlinedButton(
              onPressed: () {
                AppLogger.d(
                    '[SellerButtons] View details ${order.id}'); /* TODO: Navigate */
              },
              style: outlineStyle,
