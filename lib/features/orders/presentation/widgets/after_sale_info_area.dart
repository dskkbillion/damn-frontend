...
class AfterSaleInfoArea extends StatelessWidget {
    ...
    switch (order.state) {
      ...
      case OrderStatus.applyingForMediation:
        ...
        detailsSection = Padding(
           ...
           child: Text('您可以补充凭证或耐心等待平台处理结果。', style: textTheme.bodySmall?.copyWith(color: Colors.grey[600])), 
        );
         actionButtons = [
 ...
 Widget _buildRefundDetailsPlaceholder(BuildContext context, String status, String amount) {
   ...
   return Column(
     crossAxisAlignment: CrossAxisAlignment.start,
     children: [
       _buildDetailRow(context, '退款状态:', status, valueColor: Colors.orange[700]),
       const SizedBox(height: 8),
 ...
 Widget _buildRejectionDetailsPlaceholder(BuildContext context, String reason) {
   ...
   return Column(
     crossAxisAlignment: CrossAxisAlignment.start,
     children: [
       Text('驳回原因:', style: textTheme.labelMedium?.copyWith(color: Colors.grey[600])),
