...
/// Step 3.2 验证：profile 模块硬编码样式已替换为 DeepStream Theme Token
void main() {
  List<File> getProfilePresentationFiles() {
    final dir = Directory('lib/features/profile/presentation');
    return dir
  ...
  group('Step 3.2: Profile module reskin audit', () {
    test('No old brand color (0xFFB66D0E) remains', () {
      final files = getProfilePresentationFiles();
    ...
    test('Page files import AppColors', () {
      final pages = Directory('lib/features/profile/presentation/pages')
          .listSync()
          .whereType<File>()
          .where((f) => f.path.endsWith('.dart'))
          .toList();
      ...
      expect(count, greaterThanOrEqualTo((pages.length * 0.7).ceil()),
          reason: '$count/${pages.length} page files import AppColors');
    });

    test('Widget files import theme tokens', () {
      final widgets = Directory('lib/features/profile/presentation/widgets')
          .listSync(recursive: true)
          .whereType<File>()
          .where((f) => f.path.endsWith('.dart'))
          .toList();
      ...
      expect(count, greaterThanOrEqualTo((widgets.length * 0.7).ceil()),
          reason: '$count/${widgets.length} widget files import theme tokens');
    });
